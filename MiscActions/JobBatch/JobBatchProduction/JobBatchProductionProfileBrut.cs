...
{
    class JobBatchProductionProfileBrut : BaseJobBatchProduction, IJobBatchProduction
    {
        private static Func<ErpContext, string, string[], IEnumerable<LaborDtl>> selectLaborByJobBatch;
        public JobBatchProductionProfileBrut(Erp.ErpContext db, Epicor.Hosting.Session session) : base(db, session){}
            ...
            try
            {
                ...
                foreach (DataRow headRow in dsJobBatch.Tables["UD105"].Rows)
                {
                    ...
                    foreach(var group in groups)
                    {
                        ...
                        decimal prodQty = Math.Round((group.QteProd / 1000m), 4, MidpointRounding.AwayFromZero);
