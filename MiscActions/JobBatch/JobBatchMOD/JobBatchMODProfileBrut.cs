...
{
    class JobBatchMODProfileBrut : BaseJobBatchMOD, IJobBatchMOD
    {
        public JobBatchMODProfileBrut(Erp.ErpContext db, Epicor.Hosting.Session session) : base(db, session) { }
        ...
        public void ProcessLaborDtl(DataSet dsJobBatch, Erp.BO.CRTI_MiscAction.OperationInteraction operationInteraction, ref DataSet dsMiscAction)
        {
            ...
            foreach (DataRow row in dsJobBatch.Tables["LaborDtl"].Rows)
            {
                ...
                string jobNum = row["UD_OperJobNum_c"].ToString();
                if (operationInteraction.OperationIs("IsSetup", opCode))
                {
                    jobNum = row["UD_SetupJobNum_c"].ToString();
                }
                ...
                DataRow foundRow = dt.Rows.Find(new object[] { employeeNum, resID, jobNum, opCode, clockInDate, clockIn, clockOut });
                if (foundRow == null)
                {
                    ...
                    dt.Rows.Add(employeeNum, resID, jobNum, opCode, clockInDate, clockIn, clockOut, minutes, 0m, 0m);
                }
                ...
                if (nextDay > 0)
                {
                    ...
                    DataRow foundRow2 = dt.Rows.Find(new object[] { employeeNum, resID, jobNum, opCode, nextDate, 1, nextDay });
                    if (foundRow2 == null)
                    {
                        ...
                        dt.Rows.Add(employeeNum, resID, jobNum, opCode, nextDate, 1, nextDay, nextMinutes, 0m, 0m);
                    }
        ...
        private void GroupTime(DataTable dt, ref DataSet dsMiscAction)
        {
            ...
            {
                dtGroup.Rows.Add(group.ResourceID, group.JobNum, group.OpCode, group.MOD, group.FGF);
            }
            if (dsMiscAction.Tables.Contains("TempsRepartir"))
            {
                DataSet ds = new DataSet("MiscAction");
